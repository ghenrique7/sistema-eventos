...
@section('content')
    ...
    <form class="max-w-md mx-auto" action="{{ route('event.update', $evento) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="nome_evento"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder="" value="{{ $evento->nome_evento ?? old('nome_evento') }}" />
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <textarea id="descricao" name="descricao" rows="4"
                ...
                placeholder="Descrição"> {{ $evento->descricao ?? old('descricao') }}</textarea>
        </div>
        <div class="relative z-0 w-full mb-5 group">
            <label for="premiacao" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Premiação
                <strong>(Opcional)</strong></label>
            <textarea id="premiacao" name="premiacao" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $evento->premiacao ?? old('premiacao') }}</textarea>
        </div>
        <div class="grid md:grid-cols-2 md:gap-3">
            <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="relative flex items-center max-w-[8rem]">
                    ...
                    <input type="text" name="total_participante" id="quantity-input" data-input-counter
                        ...
                        placeholder="999" required value="{{ $evento->total_participante ?? old('total_participante') }}">
            ...
            <div class="relative z-0 w-full mb-5 group">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="imagem_arte">Upload de
                    Imagem <strong>(Opcional)</strong> </label>
                <input
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <textarea required id="kit" name="kit" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Kits do evento">{{ $evento->kit ?? old('kit') }}</textarea>
        </div>
        <div class="grid md:grid-cols-1 md:gap-6">
            <div class="relative z-0 w-full mb-5 group">
                <label for="detalhe_entrega_kit"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Detalhes da entrega do kit
                    <strong>(Opcional)</strong></label>
                <textarea id="detalhe_entrega_kit" name="detalhe_entrega_kit" rows="4"
                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Entrega do kit">{{ $evento->detalhe_entrega_kit ?? old('detalhe_entrega_kit') }}</textarea>
            </div>
        </div>
        <div class="grid md:grid-cols-2 md:gap-6 mb-4">
            <div>
                ...
                <input type="datetime-local" id="data_hora" name="data_hora" class="mt-4" required
                    value="{{ $evento->data_hora ?? old('data_hora') }}" />
            </div>
        </div>
        <div class="relative z-0 w-full mb-5 group">
            <label for="categoria" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Categoria
                <strong>(Opcional)</strong></label>
            <textarea id="categoria" name="categoria" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $evento->categoria ?? old('categoria') }}</textarea>
        </div>
        <div class="relative z-0 w-full mb-5 group">
            ...
            <textarea required id="distancia" name="distancia" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $evento->distancia ?? old('distancia') }}</textarea>
        </div>
        <div class="relative z-0 w-full mb-5 group">
            <label for="inscricao" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Inscrição <strong>(Opcional)</strong></label>
            <textarea id="inscricao" name="inscricao" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $evento->inscricao ?? old('inscricao') }}</textarea>
        </div>
        <div class="grid md:grid-cols-2 md:gap-6 mb-4">
            <div>
            ...
            <div>
                <label for="nova_modalidade" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nova
                    Modalidade <strong>(Opcional)</strong></label>
